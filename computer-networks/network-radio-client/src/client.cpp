...
void RadioClient::handle_http(HttpResponse& response) {
    ...
    if (response.status_code >= HTTP_STATUS_REDIRECT_MIN && response.status_code < HTTP_STATUS_ERROR_MIN) {
        if (response.location.empty()) {
            throw HttpException(response.status_code, "Received HTTP redirect but no 'Location' provided.");
        }
...
void RadioClient::process_meta_payload_state(size_t& offset, size_t available) {
...
void RadioClient::process_meta_length_state(size_t& offset) {
...
void RadioClient::process_audio_state(size_t& offset, size_t available) {
    ...
    while (offset < stream_data.bytes_to_process) {
        ...
        switch (stream_data.state) {
            case StreamState::READING_AUDIO:
                process_audio_state(offset, available);
                break;

            case StreamState::READING_META_LENGTH:
                process_meta_length_state(offset);
                break;

            case StreamState::READING_META_PAYLOAD:
                process_meta_payload_state(offset, available);
                break;
        }
    }
...
void RadioClient::setup_connection_if_needed(struct pollfd* pfds) {
...
bool RadioClient::execute_poll(struct pollfd* pfds) {
    ...
    if (connection.get_ms_until_timeout(timeout_ms) == 0 ) {
        ...
        if (errno == EINTR) return false; // Signal to continue loop
        int err = errno;
        throw NetworkError(err, "poll error");
    }
    
    return true; // Signal to proceed with loop execution
}

bool RadioClient::process_user_input(struct pollfd* pfds) {
...
void RadioClient::process_network_io(struct pollfd* pfds) {
    if (state != RadioState::SHUTDOWN && (pfds[POLL_NET_IDX].revents & (POLLIN | POLLOUT | POLLHUP | POLLERR))) {
        ...
        if (state == RadioState::SENDING_HTTP && (re & POLLOUT)) {
...
void RadioClient::run() {
    ...
    while (true) { 
        try {
            ...
            setup_connection_if_needed(pfds);

            // 2. Poll Configuration
            if (!execute_poll(pfds)) {
                continue; // Handles EINTR
            }

            // 3. User Input
            if (process_user_input(pfds)) {
                return; // User triggered exit
            }

            // 4. Network IO
            process_network_io(pfds);
